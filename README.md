...
```java
      ...
      setBgDarkAlpha(0.7f)                              //设置背景变暗的值 ps:如果设置的值在0 - 1的范围内，则用设置的值，否则用默认值
      enableOutsideTouchableDismiss(false)              //设置是否允许点击 PopupWindow之外的地方，关闭PopupWindow ps：默认为true 可以关闭
      build()                                           //完成构建
